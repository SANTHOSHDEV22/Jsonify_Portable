...
class JsonPathView(QWidget):
    ...
    def __init__(
        ...
        self._jsonpath_service = (
            jsonpath_service
            if jsonpath_service is not None
            else JsonPathService()
        )
    ...
    def _setup_ui(self) -> None:
        ...
        description = QLabel(
            "Query the currently loaded JSON payload using "
        ...
        self._value_preview.setLineWrapMode(
            QPlainTextEdit.LineWrapMode.NoWrap
        )

        self._value_preview.setFont(
        ...
        self._results_table.itemSelectionChanged.connect(
            self._show_selected_value
        )

        self._query_combo.lineEdit().returnPressed.connect(
            self._run_query
        )
    ...
    def set_payload(
        ...
        self._status_label.setText(
            "JSON loaded. Enter a JSONPath expression."
        )
    ...
    def _run_query(self) -> None:
        ...
        if self._payload is None:
            QMessageBox.warning(
                self,
                "No JSON Loaded",
                "Load a JSON payload before running "
                "a JSONPath query.",
            )
            return

        expression = (
            self._query_combo.currentText().strip()
        )
    ...
    def _show_selected_value(self) -> None:
        ...
        selected_rows = (
            self._results_table.selectionModel()
            .selectedRows()
        )
        ...
        self._value_preview.setPlainText(
