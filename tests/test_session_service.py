...
def test_create_session() -> None:
    ...
    assert session.workspace.selected_tab == "JSONPath"

    assert session.workspace.jsonpath_query == "$.users[*].name"
...
def test_save_session(
    ...
    assert data["format"] == "jsonify-session"
...
def test_load_session(
    ...
    loaded = service.load(file_path)

    assert loaded.name == "API Debugging"

    assert loaded.payload == original.payload

    assert loaded.workspace.selected_tab == "JSONPath"

    assert loaded.workspace.jsonpath_query == "$.users[*].name"
...
def test_invalid_json_file(
    ...
    file_path = tmp_path / "broken.jsonify"
...
def test_wrong_file_format(
    ...
    file_path = tmp_path / "wrong.jsonify"
...
def test_unsupported_version(
    ...
    file_path = tmp_path / "future.jsonify"
...
def test_missing_payload(
    ...
    file_path = tmp_path / "missing.jsonify"
